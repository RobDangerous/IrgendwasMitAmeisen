...
void Ant::init() {
	...
	vertexBuffers[1] = new Graphics4::VertexBuffer(maxAnts, *structures[1], Graphics4::StaticUsage, 1);
...
vec3 bridgeStep(Storage* storage, Bridge* bridge, int step, int maxSteps) {
	...
	P[1].y() = 5.0f;
	P[2] = island2;
	return deCasteljau(P, step / (float)maxSteps);
}

float bridgeCompleteLength(Storage* storage, Bridge* bridge) {
	float length = 0;
	vec3 last = bridgeStep(storage, bridge, 0, 100);
	for (int i = 1; i < 100; ++i) {
		vec3 temp = bridgeStep(storage, bridge, i, 100);
		length += (temp - last).getLength();
		last = temp;
	}
	return length;
}
...
void Ant::moveEverybody(Storage* storage, float deltaTime) {
	...
	for (int i = 0; i < storage->nextBridge; ++i) {
		...
		for (int a = 0; a < maxAnts; ++a) {
			if (ants[a].bridge == i) {
				ants[a].position = bridgeStep(storage, bridge, step++, bridgeCompleteStepsCount(bridge));
			}
		}
	}
...
void Ant::setLights(Kore::Graphics4::ConstantLocation lightCountLocation, Kore::Graphics4::ConstantLocation lightPosLocation) {
	static const int maxLightCount = 10;
	Kore::vec4 lightPositions[maxLightCount];

	const int lightCount = 0; // (int)room->lights.size();
